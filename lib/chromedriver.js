...
import ChromedriverStorageClient, { STORAGE_URL } from './storage-client';
...
class Chromedriver extends events.EventEmitter {
  constructor (args = {}) {
    ...
    ////////////// MODIFIED BY MO: setting timeout 3sec ///////////
    // this.jwproxy = new JWProxy({server: this.proxyHost, port: this.proxyPort});
	this.jwproxy = new JWProxy({server: this.proxyHost, port: this.proxyPort, timeout: 3000});
    //END
      ...
      if (_.isEmpty(mapping) || semver.gt(chromeVersion, _.values(mapping)[0])) {
        if (this.storageClient && !didStorageSync) {
          try {
            ...
            log.warn(`Cannot synchronize local chromedrivers with the remote storage at ${STORAGE_URL}: ` +
              e.message);
            log.debug(e);
          }
        }
